defmodule PhoenixKitAI.Web.Endpoints do
  ...
  defp build_usage_url(sort_by, sort_dir, assigns) do
    ...
    PhoenixKitAI.Routes.ai_path() <> "/usage?#{query}"
  end
  ...
  def handle_event("sort", %{"by" => field}, socket) do
    ...
    path = PhoenixKitAI.Routes.ai_path() <> "/endpoints?sort=#{field}&dir=#{sort_dir}"
  ...
  def handle_event("sort_form", params, socket) do
    ...
    path = PhoenixKitAI.Routes.ai_path() <> "/endpoints?sort=#{field}&dir=#{dir}"
    {:noreply, push_patch(socket, to: path)}
  end
  ...
  defp build_endpoints_url(sort_by, sort_dir, page) do
    base = PhoenixKitAI.Routes.ai_path() <> "/endpoints?sort=#{sort_by}&dir=#{sort_dir}"
  ...
  defp get_current_path(socket, session) do
    case socket.assigns do
      ...
      _ -> session["current_path"] || PhoenixKitAI.Routes.ai_path()
    end
  end
