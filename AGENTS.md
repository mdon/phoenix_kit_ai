...
- **LiveViews**: `use PhoenixKitWeb, :live_view` (not `Phoenix.LiveView` directly) is **not enough by itself** — every LiveView/component that calls `gettext(...)` also needs `use Gettext, backend: PhoenixKitAI.Gettext` right after it. `:live_view` imports gettext macros bound to **core's** `PhoenixKitWeb.Gettext`; without the module's own `use Gettext` line, calls silently resolve against core's catalogue instead of this module's `priv/gettext/` (no compile error — a msgid that happens to also exist in core just returns core's translation; one that doesn't just returns the raw English). Order matters too: a `gettext(...)` call written *above* the `use Gettext, backend: PhoenixKitAI.Gettext` line in the same file still binds to whichever backend was in scope at that point (i.e. core's) — put the module's own `use Gettext` line immediately after `use PhoenixKitWeb, :live_view`, before any `gettext(...)` call. Admin assigns: `@phoenix_kit_current_scope`, `@current_locale`, `@url_path`
...
- **Translatable strings**: everything user-visible through `gettext(...)`; `.po`/`.pot` files live in **this module's own** `priv/gettext/` (`PhoenixKitAI.Gettext`, `otp_app: :phoenix_kit_ai`) — not in core `phoenix_kit`. Run `mix gettext.extract && mix gettext.merge priv/gettext` after adding any new string, then fill in the `ru`/`et` `msgstr`s by hand; `mix gettext.merge` deletes any `.po` entry missing from a freshly extracted `.pot`, so a string added to one locale must go through extract+merge, not be hand-typed into the `.po` alone. Labels declared as plain string literals (`admin_tabs/0`, `permission_metadata/0`) aren't seen by `mix gettext.extract` — they're pinned via `PhoenixKitAI.translatable_labels/0` (`dgettext_noop/2`) instead. See `/www/phoenix_kit/guides/per-module-i18n.md` for the full setup and conventions.
...
- **Image editing**: `PhoenixKitAI.edit_image/4` → `Completion.edit_image/4` takes reference images (`%{data:, content_type:}` maps, `%{url:}` maps or data/http URL strings, inlined as data URLs) plus a prompt and returns `%{images: [%{data, url, content_type}], text}`. Two transports, chosen by provider: xAI posts JSON to `<base_url>/images/edits` (`image` = one `{url, type: "image_url"}` object or a list of them); every other provider goes through `<base_url>/chat/completions` with `image_url` content parts — that is how OpenRouter serves Gemini's image models, whose output arrives on `choices[0].message.images[]` as a data URL. Only OpenRouter gets `modalities: ["image","text"]` + `usage: %{include: true}` (OpenAI rejects unknown top-level fields). `:image_config` passes through on the chat path. A prose-only answer (a refusal) is `{:error, {:no_image_in_response, text}}` — the text stays in the tuple so the log shows why. Logged as `request_type: "image_edit"` with tokens + `usage.cost` when reported, `input_image_count`/`input_bytes`/`output_image_count`/`output_bytes`; the image bytes are never persisted. Never send the provider a permanent Storage URL — inline the bytes.
